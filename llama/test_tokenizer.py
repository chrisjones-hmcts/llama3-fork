...
class TokenizerTests(TestCase):
    def setUp(self):
        self.***REMOVED*** = Tokenizer(os.environ["TOKENIZER_PATH"])
        self.format = ChatFormat(self.***REMOVED***)

    def test_special_tokens(self):
        self.assertEqual(
            self.***REMOVED***.special_tokens["<|begin_of_text|>"],
            128000,
        )

    def test_encode(self):
        self.assertEqual(
            self.***REMOVED***.encode("This is a test sentence.", bos=True, eos=True),
