...
class Tokenizer:
    """
    tokenizing and encoding/decoding text using the Tiktoken ***REMOVED***.
    """
    ...
    def encode(
        ...
        *,
        ...
        # The tiktoken ***REMOVED*** can handle <=400k chars without
...
class MessageFormat:
    def __init__(self, ***REMOVED***: Tokenizer):
        self.***REMOVED*** = ***REMOVED***

    def encode_header(self, message: Message) -> List[int]:
        tokens = []
        tokens.append(self.***REMOVED***.special_tokens["<|start_header_id|>"])
        tokens.extend(self.***REMOVED***.encode(message["role"], bos=False, eos=False))
        tokens.append(self.***REMOVED***.special_tokens["<|end_header_id|>"])
        tokens.extend(self.***REMOVED***.encode("\n\n", bos=False, eos=False))
        return tokens
    ...
    def encode_message(self, message: Message) -> List[int]:
        ...
        if message.get("content", ""):
            tokens.extend(
                self.***REMOVED***.encode(message["content"].strip(), bos=False, eos=False)
            )
        tokens.append(self.***REMOVED***.special_tokens["<|eot_id|>"])
        return tokens

    def encode_dialog(self, dialog: Dialog, *, bos: bool, eos: bool) -> List[int]:
        tokens = []
        if bos:
            tokens.append(self.***REMOVED***.special_tokens["<|begin_of_text|>"])
        for message in dialog:
            tokens.extend(self.encode_message(message))

        if eos:
            # Add EOS token at the end of this dialog if required
            tokens.append(self.***REMOVED***.special_tokens["<|end_of_text|>"])
    ...
    def decode_header(self, tokens: Sequence[int]) -> ParseResult:
        ...
        role = self.***REMOVED***.decode(tokens_role)
    ...
    def decode_message(self, tokens: Sequence[int]) -> ParseResult:
        ...
        message["content"] = self.***REMOVED***.decode(tokens_content)
